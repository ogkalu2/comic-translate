...
## Corpus Check
- 345 files · ~510,560 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 5323 nodes · 15164 edges · 82 communities detected
- Extraction: 48% EXTRACTED · 52% INFERRED · 0% AMBIGUOUS · INFERRED: 7866 edges (avg confidence: 0.68)
...
1. `get()` - 383 edges
2. `TextBlock` - 255 edges
3. `MToolButton` - 185 edges
4. `TextBlockItem` - 180 edges
5. `MLabel` - 145 edges
...
- `ComicTranslate` --uses--> `Handle page changes in lazy loading webtoon mode.`  [INFERRED]
  controller.py → app\controllers\webtoons.py
- `_normalize_bulk_font_snapshot_entry()` --calls--> `get()`  [INFERRED]
  app\controllers\text_bulk_support_mixin.py → modules\utils\download.py
- `Center title in full bar width, then clamp to avoid side-widget overlap.` --uses--> `MSwitch`  [INFERRED]
  app\ui\title_bar.py → app\ui\dayu_widgets\switch.py

## Communities

### Community 0 - "Community 0"
Cohesion: 0.01
Nodes (321): create_new_blk(), create_new_txt_item(), create_path_item(), create_rect_item(), find_matching_blk(), find_matching_item(), find_matching_rect(), find_matching_txt_item() (+313 more)
...
Cohesion: 0.01
Nodes (229): extract_archive(), _get_cached_pdf(), is_image_file(), list_archive_image_entries(), make(), make_cb7(), make_cbz(), make_pdf() (+221 more)

### Community 3 - "Community 3"
Cohesion: 0.01
Nodes (191): _as_mask(), bounding_rect(), find_contours(), Image analysis operations for the imkit module., findContours-style border tracing that matches OpenCV ordering (Suzuki-style sta, OpenCV-style boundingRect replacement.      Args:         contour: np.ndarray, _trace_border_fast(), BatchExecutionMixin (+183 more)

### Community 4 - "Community 4"
Cohesion: 0.01
Nodes (211): ABC, AOT, Input image and output image have same size         image: [H, W, C] RGB, resize_keep_aspect(), DiffusionInpaintModel, forward(), init_model(), InpaintModel (+203 more)

### Community 5 - "Community 5"
Cohesion: 0.01
Nodes (109): Encode CV2/numpy image directly to base64 string using cv2.imencode., BatchReportController, BatchUiMixin, BatchReportStateMixin, BatchReportViewMixin, BatchUiMixin, _decode_ipc_messages(), ComicTranslateUI (+101 more)

### Community 6 - "Community 6"
Cohesion: 0.01
Nodes (206): contour_area(), draw_contours(), get_perspective_transform(), mean(), Calculates the area of a polygon defined by a contour using the Shoelace formula, Emulates cv2.drawContours using Pillow and Numpy.      Args:         image (n, Calculates the 3x3 perspective transform matrix using a vectorized     NumPy im, Performs a perspective warp using PIL/Pillow.          Args:         image (n (+198 more)

### Community 7 - "Community 7"
Cohesion: 0.02
Nodes (73): huge(), large(), medium(), Set the avatar size.         :param value: integer         :return: None, Set avatar image.         :param value: QPixmap or None.         :return: None, small(), tiny(), _slot_context_menu() (+65 more)

### Community 8 - "Community 8"
Cohesion: 0.02
Nodes (130): BaseLLMTranslation, encode_image(), initialize(), _merge_usage_snapshots(), _perform_translation(), Base class for LLM-based translation engines with shared functionality., Get standardized language code from language name.                  Args:, Base class for traditional translation engines (non-LLM). (+122 more)

### Community 9 - "Community 9"
Cohesion: 0.02
Nodes (137): AuthClient, Starts the new authentication flow., Handles the tokens and user info received from the backend via the local server., Handles errors emitted by the AuthServerThread., Called when the AuthServerThread finishes execution., Safely attempts to clear server thread reference., Clean up threads before application exit., Cancels the currently active authentication flow. (+129 more)

### Community 10 - "Community 10"
Cohesion: 0.02
Nodes (86): CoordinateConverter, Coordinate Converter for Webtoon Manager  Handles coordinate transformations b, Handles coordinate transformations for webtoon mode., Initializes the converter with references to the data owners.         This clas, Convert a scene-coordinate brush path to page-local coordinates., Convert a page-local brush path to scene coordinates., FullscreenResultPreview, LazyImageLoader (+78 more)

### Community 11 - "Community 11"
Cohesion: 0.02
Nodes (117): detect(), DetectionEngine, Abstract base class for all detection engines.     Each model implementation sh, Export cache contents in a project-serializable form., detect_content_in_bbox(), filter_and_fix_bboxes(), _process_stats_vectorized(), Content detection and processing utilities. (+109 more)

### Community 12 - "Community 12"
Cohesion: 0.03
Nodes (42): EdgeResizer, _edges_at(), Return a Qt.Edges flag for whichever window edges *gpos* is within *margin* pixe, Event filter that provides edge resize cursors and startSystemResize for framele, MCacheDict, MainWindowBuildersMixin, MSplitter, MSwitch (+34 more)
...
Cohesion: 0.04
Nodes (37): _encode_ipc_message(), _extract_project_file(), FileOpenEventFilter, get_system_language(), LoadingWorker, main(), OpenRequestRouter, Do the heavy loading in background thread. (+29 more)
...
Cohesion: 0.06
Nodes (26): _pixels_to_qfont_points(), Convert image pixel sizing to QFont point sizing., dedent(), indent(), _munge_whitespace(text : string) -> string          Munge whitespace in text:, _split(text : string) -> [string]          Split the text to wrap into indivis, _fix_sentence_endings(chunks : [string])          Correct for sentence endings, _handle_long_word(chunks : [string],                             cur_line : [st (+18 more)
...
Cohesion: 0.15
Nodes (12): __init__(), MBaseButton, MDBPathButtons, parse_db_orm(), parse_path(), slot_action_clicked(), slot_button_clicked(), slot_go_to() (+4 more)

### Community 23 - "Community 23"
Cohesion: 0.21
Nodes (2): MFlowLayout, FlowLayout, the code is come from PySide/examples/layouts/flowlayout.py     I c
...
Cohesion: 0.43
Nodes (7): _iter_imported_modules(), _iter_python_files(), _parse_python(), test_no_empty_wrapper_modules(), test_no_imports_of_removed_modules(), test_no_manual_workflow_symbol_mentions(), test_removed_files_are_gone()
...
## Knowledge Gaps
- **621 isolated node(s):** `Checks for updates on GitHub and handles downloading/running installers.`, `Starts the check in a background thread.`, `Starts the download in a background thread.`, `Executes the installer based on the platform.`, `Stops any active worker thread (best-effort).` (+616 more)
...
- **Why does `get()` connect `Community 2` to `Community 0`, `Community 1`, `Community 3`, `Community 4`, `Community 5`, `Community 6`, `Community 7`, `Community 8`, `Community 9`, `Community 10`, `Community 11`, `Community 12`, `Community 13`, `Community 16`, `Community 17`, `Community 20`, `Community 22`?**
  _High betweenness centrality (0.225) - this node is a cross-community bridge._
- **Why does `TextBlock` connect `Community 4` to `Community 0`, `Community 2`, `Community 3`, `Community 5`, `Community 6`, `Community 8`, `Community 11`, `Community 19`?**
  _High betweenness centrality (0.090) - this node is a cross-community bridge._
- **Why does `MLabel` connect `Community 1` to `Community 0`, `Community 5`, `Community 7`?**
  _High betweenness centrality (0.069) - this node is a cross-community bridge._
- **Are the 375 inferred relationships involving `get()` (e.g. with `get_system_language()` and `load_translation()`) actually correct?**
  _`get()` has 375 INFERRED edges - model-reasoned connections that need verification._
- **Are the 241 inferred relationships involving `TextBlock` (e.g. with `ComicTranslate` and `Wrap thread_load_images with unsaved-project confirmation and clear state.`) actually correct?**
  _`TextBlock` has 241 INFERRED edges - model-reasoned connections that need verification._
...
- **Are the 114 inferred relationships involving `TextBlockItem` (e.g. with `ComicTranslate` and `Wrap thread_load_images with unsaved-project confirmation and clear state.`) actually correct?**
  _`TextBlockItem` has 114 INFERRED edges - model-reasoned connections that need verification._