...
from modules.detection import get_inpaint_bboxes
...
class AddRectangleCommand(QUndoCommand, RectCommandBase):
    ...
    def redo(self):
        if not self.find_matching_rect(self.scene, self.rect_properties):
            ...
            self.viewer._rectangles.append(rect_item)
    ...
    def undo(self):
        ...
        if matching_item:
            ...
            self.viewer._rectangles.remove(matching_item)
    ...
    def redo(self):
        ...
        for item in self.scene.items():
            if isinstance(item, MoveableRectItem) and item != self.photo:
                ...
                self.viewer._selected_rect = None
        self.scene.update()
...
class DeleteBoxesCommand(QUndoCommand, RectCommandBase):
    ...
    def redo(self):
        ...
        if matching_rect:
            ...
            self.viewer._rectangles.remove(matching_rect)
            self.viewer._selected_rect = None
            self.scene.update()
        ...
        if matching_txt_item:
            ...
            self.viewer._text_items.remove(matching_txt_item)
    ...
    def undo(self):
        ...
        if self.txt_item_prp and not self.find_matching_txt_item(self.scene, self.txt_item_prp):
            ...
            self.viewer._text_items.append(text_item)
