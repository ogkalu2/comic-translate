...
class AuthClient(QObject): 
    ...
    def _handle_token_callback(self, access_token: str, refresh_token: str, user_info: dict):
        ...
        try:
            ...
            keyring.set_password("comic-translate", "access_token", access_token)
            # Always store refresh token if received
            if refresh_token:
                keyring.set_password("comic-translate", "refresh_token", refresh_token)
            else:
                # Handle case where refresh token might not be provided (though it should be)
                logger.warning("Refresh token was not received from backend callback.")
                try: keyring.delete_password("comic-translate", "refresh_token")
    ...
    def refresh_token(self) -> bool:
        ...
        try:
            refresh_token = keyring.get_password("comic-translate", "refresh_token")
            access_token = keyring.get_password("comic-translate", "access_token")
            ...
            keyring.set_password("comic-translate", "access_token", new_access_token)
            if new_refresh_token:
                logger.info("Refresh token rotated. Storing new refresh token.")
                keyring.set_password("comic-translate", "refresh_token", new_refresh_token)
    ...
    def logout(self):
        ...
        try:
            ...
            keyring.delete_password("comic-translate", "access_token")
        except Exception as e:
        ...
        try:
            keyring.delete_password("comic-translate", "refresh_token")
        except Exception as e:
    ...
    def fetch_user_info(self):
        ...
        try:
            token = keyring.get_password("comic-translate", "access_token")
            if not token:
