...
class BatchReportStateMixin:
    def _refresh_latest_report(
        ...
        *,
        ...
        report = {
            "started_at": current_report.get("started_at", latest_report.get("started_at", now)),
            "finished_at": now,
            "was_cancelled": bool(latest_report.get("was_cancelled", False) if was_cancelled is None else was_cancelled),
        ...
        if self._batch_report_drawer is not None:
            try:
                self._batch_report_drawer.set_widget(self._build_batch_report_widget(report))
            except Exception:
                pass
    ...
    def start_batch_report(self, batch_paths: list[str]):
        ...
        self._current_batch_report = {
            ...
            "path_to_page_number": {path: index + 1 for index, path in enumerate(tracked_paths)},
            "skipped": {},
        }
    ...
    def _is_content_flagged_error_text(self, error: str) -> bool:
        ...
        return "flagged as unsafe" in lowered or "content was flagged" in lowered or "safety filters" in lowered
    ...
    def _localize_batch_skip_detail(self, error: str) -> str:
        ...
        if "connection" in lowered or "network" in lowered or "name or service not known" in lowered or "failed to establish" in lowered:
            return self.main.tr("Network or connection error")
        if "bad gateway" in lowered or "service unavailable" in lowered or "gateway timeout" in lowered or "502" in lowered or "503" in lowered or "504" in lowered:
            return self.main.tr("Provider unavailable")
        if "jsondecodeerror" in lowered or "empty json" in lowered or "expecting value" in lowered:
    ...
    def _format_batch_skip_reason(self, skip_reason: str, error: str) -> str:
        ...
        reason_map = {
            ...
            "Translation Chunk Failed": self.main.tr("Webtoon translation chunk failed"),
        }
    ...
    def register_batch_skip(self, image_path: str, skip_reason: str, error: str):
        ...
        if not report or image_path not in report["path_set"]:
            return
        ...
        if existing is None:
            entry = self._make_error_entry(image_path, [reason_text] if reason_text else [])
            entry["page_number"] = report["path_to_page_number"].get(image_path, 0)
            report["skipped"][image_path] = entry
            self._error_registry[image_path] = self._make_error_entry(image_path, [reason_text] if reason_text else [])
            self._refresh_latest_report(update_buttons=False)
            return

        if reason_text and reason_text not in existing["reasons"]:
            existing["reasons"].append(reason_text)
        self._error_registry[image_path] = self._make_error_entry(image_path, existing["reasons"])
        self._refresh_latest_report(update_buttons=False)
    ...
    def _build_current_error_entries(self) -> list[dict]:
        ...
        for image_path, entry in self._error_registry.items():
            if image_path not in self.main.image_files:
                ...
                continue
            active_entries.append(self._make_error_entry(image_path, entry.get("reasons", [])))

        for image_path in stale_paths:
            self._error_registry.pop(image_path, None)

        active_entries.sort(key=lambda entry: (entry.get("page_number", 0), entry["image_name"].lower()))
    ...
    def finalize_batch_report(self, was_cancelled: bool):
        ...
        if not report:
            return None
        return self._refresh_latest_report(update_buttons=True, was_cancelled=was_cancelled)
